    ...
    getConnectionsToDatabases(cookies.token)
      .then((response) => {
        const connection = response.data.find(
          (connection: ConnectionItem) => connection.id === parseInt(id),
        );
      ...
      <IonContent>
        <IonHeader>
          <IonToolbar>
            ...
            <IonTitle>
              {capitalize(currentConnection?.erp_system || '')} {t('text.connection')}
            </IonTitle>
            <IonButton
              slot="end"
              size="small"
        ...
        <Input
          ...
          value={currentConnection?.erp_system || ''}
          required
          handleChange={() => setChanged(true)}
        />
      </IonContent>
