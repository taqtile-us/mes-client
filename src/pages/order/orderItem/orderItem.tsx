  ...
  useIonViewWillEnter(() => {
    ...
    if (operationId) {
      ORDER_REQUEST.getOrderOperationById(
        operationId,
        setOperation as React.Dispatch<SetStateAction<IOrderOperation>>,
        setLoading,
        setToastMessage,
      );
    }
  ...
  const filteredRows = useMemo(
    () =>
      tableItems.filter(
        (row) =>
          row.values[1] &&
          row.values[1].toString().toLocaleLowerCase().includes(searchText.toLocaleLowerCase()),
      ),
