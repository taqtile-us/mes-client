...
export const OrdersPage: React.FC = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState<OperationStatus>(
    currentStatus as OperationStatus,
  );
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((item) => {
      ...
      const searchFilter = concatOrderNumberName(item)
        ?.toLowerCase()
  ...
  return (
    <IonPage>
      <Header
        ...
        endButton={
          <img src={Chart} alt="orders view" onClick={() => handleItemClick(ROUTES.ORDERSVIEW)} />
        }
      />
              ...
              {filteredOrders.map((item) => (
                ...
                >
                  <div className={styles.orderCardItems}>
                    {checkDeadline(item) && (
                      <div className={styles.deadlineExpired}>{checkDeadline(item)}</div>
                    )}
                    {item.startedAt && (
                      <div>
