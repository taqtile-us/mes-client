      ...
      selectedSegment === TIMESPAN_STATES.INPROCESS
        ? timespans.filter(
            (timespan) =>
              ...
              timespan.orderOperation.orderItem.name
                .toLowerCase()
      ...
      <IonContent>
        ...
        <IonList>
          {filteredTimespans.length > 0 ? (
            filteredTimespans.map((timespan) => (
              <IonItem
                key={timespan.timespanId}
                className="timespan-item"
                onClick={() => {
                  if (selectedSegment === TIMESPAN_STATES.INPROCESS) {
                    history.push(ROUTES.EMPLOYEE_TASK_EDIT(String(timespan.timespanId)), {
                      orderName: timespan.orderOperation.orderItem.name,
                    });
                  } else {
                    history.push(ROUTES.EMPLOYEE_TASK_GET(String(timespan.timespanId)), {
                      orderName: timespan.orderOperation.orderItem.name,
                    });
                  }
                }}
                ...
                <IonLabel className="timespan-label">
                  ...
                  <div className="timespan-item-columns">
                    <div className="timespan-item-column">
                      <IonIcon slot="start" style={{ fontSize: '24px' }} icon={Calendar} />
                      <p>{formatDateWithFullMonthName(timespan.startedAt, currentLanguage)}</p>
                    </div>
                    <div className="timespan-item-column">
                      <IonIcon slot="start" style={{ fontSize: '24px' }} icon={Clock} />
                      <p>
                        {extractTime(timespan.startedAt)} -{' '}
                        {timespan.finishedAt ? extractTime(timespan.finishedAt) : ''}
                      </p>
                    </div>
                  </div>
                </IonLabel>
        ...
        {selectedSegment === TIMESPAN_STATES.INPROCESS && filteredTimespans.length === 1 && (
          <div className="ion-padding">
            <IonButton expand="block" onClick={() => openFinishModal(filteredTimespans[0])}>
              {t('operations.finish')}
            </IonButton>
          </div>
        )}
