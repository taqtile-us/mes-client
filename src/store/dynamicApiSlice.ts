...
export const dynamicApiSlice = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: (headers) => {
      ...
      if (token) {
        // console.log(token)
        headers.set('Authorization', `${token}`), headers.set('ngrok-skip-browser-warning', 'true');
      }
      return headers;
    },
  }),
  endpoints: (builder) => ({
    ...
    getCollectionItem: builder.query<{ [key: string]: any }[], any>({
      query: (params: { meta: CollectionMeta; itemId: number }) => ({
        url: params.meta.url + `${params.itemId}/`,
        method: 'GET',
      }),
    }),
